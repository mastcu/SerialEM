...
void CSubclassWndMap::Add(HWND hwnd, CSubclassWnd* pSubclassWnd)
{
	...
	if (pSubclassWnd->m_pNext==NULL) {
		...
		pSubclassWnd->m_pOldWndProc = 
			(WNDPROC)(INT_PTR)SetWindowLongPtr(hwnd, GWLP_WNDPROC, (LONG_PTR)HookWndProc); //TODO I changed this 
...
void CSubclassWndMap::Remove(CSubclassWnd* pUnHook)
{
	...
	if (pHook==pUnHook) {
		...
		if (pHook->m_pNext)
			...
			RemoveKey(hwnd);
			SetWindowLongPtr(hwnd, GWLP_WNDPROC, (LONG_PTR)pHook->m_pOldWndProc); //TODO I changed this 
		}
	} else {
