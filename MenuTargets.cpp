...
BEGIN_MESSAGE_MAP(CMenuTargets, CCmdTarget)
  ...
  ON_UPDATE_COMMAND_UI(ID_TASKS_ASSESSANGLERANGE, OnUpdateNoTasksNoTSnoSTEM)
...
{
  mWinApp->mNavigator->RealignToCurrentItem(true);
}
...
void CMenuTargets::OnCalibrationListisvectors() 
{
  ...
  for (int actCam = 0; actCam < numCam; actCam++) {
    iCam = active[actCam];
    str = CString("\r\nImage shift calibration vectors for ") + camP[iCam].name;
...
void CMenuTargets::OnUpdateCameraNormalizehere(CCmdUI* pCmdUI) 
{
  ...
  pCmdUI->Enable(!param->TietzType && !param->AMTtype && (!param->DE_camType || 
    param->DE_camType >= 2) &&
    !param->K2Type && !param->OneViewType && (param->pluginName.IsEmpty() || 
