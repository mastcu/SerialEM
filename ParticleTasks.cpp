...
int CParticleTasks::StartOneShotOfMulti(void)
{
  ...
  bool earlyRet = ((mMSIfEarlyReturn == 1 && !GetNextShotAndHole(nextShot, nextHole)) ||
    mMSIfEarlyReturn == 2 || (mMSIfEarlyReturn > 2 && (mMSHoleIndex > 0 ||
      ...
      (camParams->FEItype == FALCON4_TYPE && mCamera->GetFalconCanReturnEarly()) ||
        mCamera->ThisDEcanReturnEarly(camParams)) && !mMSDoStartMacro;
...
void CParticleTasks::TemplateAlignNextTask(int param)
{
  ...
  if (mATResettingShift) {
    ...
    if (mATParams.leaveISatZero &&
      ...
      if (mATFinishing)
        PrintfToLog("%s finished after %d IS resets with IS left at zero", 
          mATHoleCenteringMode == 0 ? "Align to Template" : "Center on Hole",
          mATIterationNum);
      else
        PrintfToLog("%s stopped at limit of %d iterations, with IS left at 0",
          mATHoleCenteringMode == 0 ? "Align to Template" : "Center on Hole", 
          mATIterationNum);
      StopTemplateAlign();
      return;
      ...
      if (mATIterationNum == 0) {
        ...
        if (ix0) {
          if (ix0 == FCH_ERR_CROP_RANGE)
            ...
            mess.Format("Error: image shift to center on hole is above the set limit");
          else 
