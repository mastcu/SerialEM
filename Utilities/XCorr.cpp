                    ...
                    int invertCon, short int *brray, int *nxout, int *nyout)
{
  ...
  unsigned short *ualine8, *ubline8, *ualineStart, *ublineStart;
  ...
  *nyout = ny;
  ...
  if (invertCon) {
    if (type == SIGNED_SHORT) {
      for (ix = 0; ix < nx * ny; ix++) {
        dmax = B3DMAX(array[ix], dmax);
        dmin = B3DMIN(array[ix], dmin);
      }
    } else {
      uarray = (unsigned short *)array;
      for (ix = 0; ix < nx * ny; ix++) {
        dmax = B3DMAX(uarray[ix], dmax);
        dmin = B3DMIN(uarray[ix], dmin);
      }
    }
  }
  ...
  alineStart = array;
  numStrips = ny / 8;
  if (invertCon == 0 || type == SIGNED_SHORT) {

    // Signed shorts or no inversion
    ...
    // Unsigned shorts with inversion
    ublineStart = (unsigned short *)blineStart;
    ualineStart = (unsigned short *)alineStart;
    for (strip = 0, iy = 0; strip < numStrips; strip++, iy += 8) {
      ualine1 = ualineStart;
      ualine2 = ualine1 + nx;
      ualine3 = ualine2 + nx;
