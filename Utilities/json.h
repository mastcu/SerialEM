...
namespace json
{
	...
	namespace parsing
	{
      ...
      // DNM modified to compile in Windows with v140_xp toolset and use large buffer
      // instead of reallocating to a buffer buffer
      char buffer[64];
			int remainder = snprintf(&buffer[0], 64, format, number);
			if(remainder < 0)
				return std::string();
